...
function injectAfter(source, anchor, addition, label) {
  ...
  if (index === -1) throw new Error(`Xray native injection failed: cannot find ${label}`);
...
function prepare({ repoRoot }) {
  ...
  patch(ruleset, source => {
    source = injectAfter(
      ...
      "\nimport { XrayRuleSet } from '../../../ForkExtras/Clients/native/xray';",
      'RulesetOutput import anchor'
    );
    return injectAfter(
      source,
      '      new SurgeRuleSet(type),',
      '\n      new XrayRuleSet(type),',
  ...
  patch(domainset, source => {
    source = injectAfter(
      ...
      "\nimport { XrayDomainSet } from '../../../ForkExtras/Clients/native/xray';",
  ...
  patch(strategyWriteData, source => {
    source = injectAfter(
      ...
      "\nimport { XrayDomainSet, XrayRuleSet } from '../../../ForkExtras/Clients/native/xray';",
      'strategy worker import anchor'
    );
    return injectAfter(
      source,
      "  'surge ruleset': (d) => new SurgeRuleSet(d.type, d.outputDir),",
      "\n  'xray domainset': (d) => new XrayDomainSet(d.outputDir),\n  'xray ruleset': (d) => new XrayRuleSet(d.type as 'non_ip' | 'ip', d.outputDir),",
