...
function injectAfter(source, anchor, addition, label) {
  ...
  if (index === -1) throw new Error(`Loon native injection failed: cannot find ${label}`);
...
function prepare({ repoRoot }) {
  ...
  patch(ruleset, (source) => {
    source = injectAfter(
      ...
      "\nimport { LoonRuleSet } from '../../../ForkExtras/Clients/native/loon';",
      'RulesetOutput import anchor'
    );
    return injectAfter(
      source,
      '      new SurgeRuleSet(type),',
      '\n      new LoonRuleSet(type),',
  ...
  patch(domainset, (source) => {
    source = injectAfter(
      ...
      "\nimport { LoonDomainSet } from '../../../ForkExtras/Clients/native/loon';",
  ...
  patch(strategyWriteData, (source) => {
    source = injectAfter(
      ...
      "\nimport { LoonDomainSet, LoonRuleSet } from '../../../ForkExtras/Clients/native/loon';",
      'strategy worker import anchor'
    );
    source = injectAfter(
      source,
      "  'surge ruleset': (d) => new SurgeRuleSet(d.type, d.outputDir),",
      "\n  'loon domainset': (d) => new LoonDomainSet(d.outputDir),\n  'loon ruleset': (d) => new LoonRuleSet(d.type as 'non_ip' | 'ip', d.outputDir),",
